...
test.describe('Analytics - Admin Filtering (CRITICAL)', () => {
  ...
  test('public analytics query should exclude admin events', async ({ page }) => {
    ...
    const { data: publicEvents, count: publicCount } = await supabase
      .from('analytics_page_views')
      .select('*', { count: 'exact' })
      .eq('session_id', adminTestSessionId)
      .eq('is_admin', false)
  ...
  test('percentage of admin events should be reasonable', async ({ page }) => {
    ...
    if (totalEvents && totalEvents > 0) {
      ...
      if (adminPercentage > 50) {
        console.warn(
          '[Test] ⚠️  Admin events are >50% of total - this is unusual for production'
        )
      }

...
test('admin events are marked correctly', async ({ page }) => {
  ...
  const allPageViews = await supabase.from('analytics_page_views').select('count')
...
- **feature\_\***: Feature usage tracking
...
export async function GET() {
  try {
    ...
    const [pageViews, searches, resourceEvents, mapEvents, featureEvents, performanceEvents] =
      await Promise.all([
        supabase
          .from('analytics_page_views')
    ...
    return NextResponse.json({
      ...
      admin_filtering: {
        ...
        admin_percentage:
          totalEvents > 0 ? (((adminEvents.count || 0) / totalEvents) * 100).toFixed(1) : '0.0',
      },
      errors: {

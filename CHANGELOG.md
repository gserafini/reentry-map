...
- Replace the generic framework social preview with branded, page-specific Open Graph cards that show the shared location, service type, provider, and listing count when available.
- Preserve deliberate map zoom and pan changes while the page publishes pending viewport bounds, including location-and-radius searches that previously reapplied their preset zoom after every map interaction.
...
- Add guest saved contacts on the current device, printable/downloadable support lists, shared-device removal controls, and genuine offline contact access. Factual corrections can be submitted without an account and remain moderated, bounded, and rate limited.
- Add optional search-journey measurement with raw queries and exact coordinates excluded. Honor tracking preference and browser privacy signals.
- Validate cached and detected locations in one place, prevent delayed GPS/GeoIP responses from overriding manual choices, and discard malformed saved coordinates.
- Collect MUI styles through the Next.js App Router cache to prevent hydration mismatches. Bound static build workers to protect the shared database, and make the console gate fail on actual HTTP, rendering, and application errors while cleaning up only its own temporary server.
- Escape embedded structured data and bound city metadata queries; improve contrast and filter-dialog accessibility. Use the public site origin for share-preview metadata when no explicit environment override is configured.
- Upgrade NextAuth to 4.24.15 so malformed Bearer headers are rejected without triggering authentication failures.
- Harden exact-revision local deployments and guarded public-resource refreshes for staging; keep builds and runtime rollout together.

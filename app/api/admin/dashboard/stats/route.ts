  ...
  try {
    ...
    if (shouldFetch('costs')) {
      queries.push(
        sql<CostSummary[]>`
          SELECT
            COALESCE(SUM(total_cost_usd) FILTER (WHERE created_at >= date_trunc('month', NOW())), 0)::float as monthly_cost,
            COALESCE(SUM(total_cost_usd) FILTER (WHERE created_at >= NOW() - interval '7 days'), 0)::float as weekly_cost,
            COALESCE(SUM(total_cost_usd) FILTER (WHERE created_at >= NOW() - interval '1 day'), 0)::float as daily_cost
          FROM ai_usage_logs
        `.then((rows) => {
      ...
      queries.push(
        sql<CostBreakdownRow[]>`
          SELECT
            COALESCE(operation, 'other') as operation,
            SUM(total_cost_usd)::float as cost,
            COUNT(*)::int as count
          FROM ai_usage_logs
          WHERE created_at >= date_trunc('month', NOW())
          GROUP BY COALESCE(operation, 'other')
          ORDER BY cost DESC
        `.then((rows) => {
    ...
    if (shouldFetch('logs')) {
      queries.push(
        sql<AgentLogRow[]>`
          SELECT id, agent_type, status, resources_processed, resources_added,
                 resources_updated, cost_cents, error_message, created_at, completed_at
          FROM ai_agent_logs
    ...
    // Flagged resource count
    if (shouldFetch('flagged')) {
      queries.push(
        sql<FlaggedCounts[]>`
          SELECT COUNT(*)::int as count
          FROM resources
          WHERE flagged = true AND flag_resolved = false
        `.then((rows) => {
      ...
      queries.push(
        Promise.all([
          ...
          sql<AgentSessionRow[]>`
            ...
            LIMIT 10
          `,
        ]).then(([resources, suggestions, updates, agentSessions]) => {
          results.activity = {
            resources: resources || [],
    ...
    // Verification events (for RealtimeVerificationViewer)
    if (shouldFetch('verification')) {
      const oneHourAgo = new Date(Date.now() - 60 * 60 * 1000).toISOString()
      queries.push(
        sql<VerificationEventRow[]>`
    ...
    // Active agent sessions (full details for command center)
    if (shouldFetch('activeAgentSessions')) {
      queries.push(
        sql<ActiveAgentSessionRow[]>`

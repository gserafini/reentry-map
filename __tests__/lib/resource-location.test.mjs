...
describe('resource location helpers', () => {
  ...
  it('describes declared statewide eligibility rather than the map anchor state', () => {
    expect(
      getServiceAreaSummary({
        address_type: 'regional',
        state: 'HI',
        service_area: { type: 'statewide', values: ['TX'] },
      })
    ).toBe('Serves all of Texas')
  })

  it('describes every declared state and normalizes state codes', () => {
    expect(
      getServiceAreaSummary({
        address_type: 'regional',
        state: 'CA',
        service_area: { type: 'statewide', values: ['TX', 'Hawaii', 'TX'] },
      })
    ).toBe('Serves all of Texas and Hawaii')
  })
